...
class Pompier
{
  ...
  public function OnePompierByID($id)
  {
    ...
    $sql = "SELECT P_ID,P_CODE,P_NOM, P_PRENOM, P_PRENOM2, P_SEXE, P_CIVILITE , P_GRADE,G_DESCRIPTION, GP_ID, GP_ID2 FROM `pompier` JOIN grade on P_GRADE = G_GRADE  WHERE P_ID = " . $id . ";";
    $dbh = BDD::getInstance();
    $stmt = $dbh->prepare($sql);
    $stmt->execute();
    return $stmt;
